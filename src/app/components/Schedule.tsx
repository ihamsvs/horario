        ...
        <table className="w-full min-w-[400px]">
          <thead>
            <tr className="border-b-2 border-black bg-red-500">
              <th className="py-2 sm:py-3 px-2 sm:px-4 text-left text-xs sm:text-sm font-black text-white uppercase">
                Día
              </th>
              <th className="py-2 sm:py-3 px-2 sm:px-4 text-left text-xs sm:text-sm font-black text-white uppercase">
                Asignaturas
          ...
          <tbody>
            {daysOfWeek.map((day, index) => (
              <tr
                key={day}
                className={`border-b-2 border-black transition-all duration-500 ease-in-out ${
                  index % 2 === 0 ? 'bg-white' : 'bg-gray-100'
                } ${
                  day === currentDay
                    ? 'bg-yellow-100 animate-pulse' // Resaltar el día actual
                    : 'hover:bg-gray-200' // Efecto hover para otros días
                }`}
              >
                <td className="py-2 sm:py-3 px-2 sm:px-4 text-xs sm:text-sm font-bold text-black uppercase border-r-2 border-black">
                  {day}
                </td>
